...
// };
export function mapPromise(values: Array<any>, converter: any, option?: any) {
...
export default class ProofsUtil {
  ...
  static async buildBlockProof(web3, start, end, blockNumber) {
    ...
    const proof = await ProofsUtil.getFastMerkleProof(
      web3,
      blockNumber,
      start,
      end
  ...
  static async buildBlockProofHermoine(
    web3,
    start,
    end,
    blockNumber,
    ...
    logger.debug('buildBlockProof...', start, end, blockNumber);
    const tree = await ProofsUtil.buildBlockHeaderMerkleHermoine(
      start,
      end,
      networkApiUrl
    );
  ...
  static async queryRootHash(web3: any, startBlock: number, endBlock: number) {
    try {
      return ethUtils.toBuffer(
        `0x${await web3.bor.getRootHash(startBlock, endBlock)}`
      );
    } catch (err) {
      return null;
    }
  ...
  static async buildBlockHeaderMerkleHermoine(start, end, networkApiUrl) {
    const logDetails = await axios.get(
      networkApiUrl + '/generate-proof?start=' + start + '&end=' + end
    );
    const logs = logDetails.data.merkle_headerblocks;
  ...
  static async getTxProof(tx, block) {
    ...
    const stateSyncTxHash = ethUtils.bufferToHex(
      ProofsUtil.getStateSyncTxHash(block)
    );
  ...
  static getStateSyncTxHash(block): Buffer {
    return ethUtils.keccak256(
      Buffer.concat([
        ethUtils.toBuffer('matic-bor-receipt-'.toString('hex')), // prefix for bor receipt
