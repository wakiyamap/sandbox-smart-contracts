...
describe('PolygonAvatar.sol differences with Avatar.sol', function () {
  ...
  describe('pause/unpause', function () {
    it('should fail to deposit when paused and success when unpaused', async function () {
      const fixtures = await setupAvatarTest();
      const avatarAsChildChainManager = await ethers.getContract(
        'PolygonAvatar',
        fixtures.childChainManager
      ...
      await fixtures.polygonAvatarAsPauser.pause();
      await expect(
        avatarAsChildChainManager.deposit(fixtures.other, depositData)
    ...
    it('should fail to withdraw when paused and success when unpaused', async function () {
      const fixtures = await setupAvatarTest();
      const tokenId = BigNumber.from('0xdada1');
      ...
      await fixtures.polygonAvatarAsPauser.pause();
      await expect(
        fixtures.polygonAvatarAsOther.withdraw(tokenId)
      ).to.revertedWith('paused');
    ...
    it('should fail to withdrawBatch when paused and success when unpaused', async function () {
      const cant = 3;
      const baseId = BigNumber.from('0xdada1');
      const fixtures = await setupAvatarTest();
      ...
      await fixtures.polygonAvatarAsPauser.pause();
      await expect(
        fixtures.polygonAvatarAsOther.withdrawBatch(tokenIds)
      ).to.revertedWith('paused');
